...
public class BitwiseAndParserChainNodeTest extends ParserChainNodeTestBase {
    ...
    private BitwiseAndParserChainNode bitwiseAndParserChainNode;

    @BeforeEach
    void setUp() {
        bitwiseAndParserChainNode = new BitwiseAndParserChainNode();
        bitwiseAndParserChainNode.setNext(next).setRoot(root);
    }
    ...
    public void shouldReturnNextResultWhenDoesNotMatchOperator() {
        ...
        Expression expression = bitwiseAndParserChainNode.parse(tokenHolder);

        assertEquals(firstResult, expression, () -> "Expected " + bitwiseAndParserChainNode + " to return next node's" +
    ...
    public void shouldReturnBinaryExpressionWithAndOperator() {
        ...
        assertBinaryExpressionWithChildren(expression, firstResult, secondResult, BinaryExpression.Operation.AND);

        verify(tokenHolder, times(2)).match(TokenType.AMP);
    ...
    public void shouldReturnNestedBinaryExpressions() {
        ...
        BinaryExpression leftBinaryExpression =
                assertBinaryExpressionWithChildren(left, firstResult, secondResult, BinaryExpression.Operation.AND);

        assertBinaryExpressionChildren(binaryExpression, leftBinaryExpression, thirdResult);

        verify(tokenHolder, times(3)).match(TokenType.AMP);
